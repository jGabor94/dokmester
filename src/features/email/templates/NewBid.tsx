...
export const NewBidEmail = (doc: SelectDocument, customHtml?: string) => (
  <Tailwind config={tailwindEmailConfig} >
    <Container className={`bg-slate-50 text-slate-950 my-0 mx-auto`}>
      ...
      <Section className={`bg-slate-50 p-6`}>
        ...
        <Text className={`m-0 mb-4 font-bold`}>Új árajánlatot állítottak ki az Ön részére!</Text>
        <Text className={`m-0 mb-1`}>Kiállító adatai:</Text>
        <Text className={`m-0`}>{doc.data.issuer.name}</Text>
        <Text className={`m-0`}>{doc.data.issuer.zip} {doc.data.issuer.city}, {doc.data.issuer.address}</Text>
        <Text className={`m-0 mb-6`}>{doc.data.issuer.taxnumber}</Text>
        <Text className={`m-0`}>Az árajánlat teljes tartalmát a mellékletben csatolt PDF fájl tartalmazza!</Text>
