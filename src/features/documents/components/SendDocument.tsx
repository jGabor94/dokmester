...
import { SA_SendDocumentMail } from '@/features/email/actions'
...
const SendDocument: FC<{ document: SelectInvoice }> = ({ document }) => {
  ...
  const handlePDFSend = async () => {
    if (editor) {
      ...
      const res = await SA_SendDocumentMail(document.id, !editor.isEmpty ? editor.getHTML() : "");
